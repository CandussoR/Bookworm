<template>
  <main class="flex flex-col align-center">
    ...
    <div class="mt-5 bg-base-200 py-2 rounded">
      <div
        id="drag-drop-inside"
        class="flex flex-col border-2 rounded m-5 p-10 items-center justify-center text-center"
      >
        <svg
    ...
    <div class="mt-5" ref="table">
      <h3 class="text-lg text-bold text-center" v-if="added && Object.keys(added).length > 0">
        Waiting for review
      </h3>

      <EditableMetadataTable
        v-if="added && Object.keys(added).length > 0"
        :ebooks="added"
        @deleted="deleteFromAdded($event)"
        @updated="store.updateAddingList($event)"
      />
    </div>
  </main>
</template>
