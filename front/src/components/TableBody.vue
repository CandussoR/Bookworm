<template>
  <tbody>
    <tr v-for="(e, i) in props.ebooks" :key="e.ebook_guid"
      ...
      <td><input type="checkbox" value="false" @change="handleSelection(i)"/> {{props.selected && props.selected.includes(i)}}</td>
      <td>{{ e.title }}</td>
      <td>
...
const emit = defineEmits(['get-ebooks-for', 'update-selection'])
...
function handleSelection(i) {
  if (selected.value.length && selected.value.includes(i)) {
